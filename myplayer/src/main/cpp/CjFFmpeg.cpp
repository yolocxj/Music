...
void CjFFmpeg::decodeFFmpegThread() {
    ...
    for(int i = 0; i < pFormatCtx->nb_streams; i++)
    {
        ...
        else if(pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            if(video == NULL)
            {
                video = new CjVideo(playstatus, callJava);
                video->streamIndex = i;
...
void CjFFmpeg::start() {
    ...
    if(supportMediacodec)
    {
        ...
        video->cjCallJava->onCallInitMediacodec(
                codecName,
...
int CjFFmpeg::getCodecContext(AVCodecParameters *codecpar, AVCodecContext **avCodecContext) {
